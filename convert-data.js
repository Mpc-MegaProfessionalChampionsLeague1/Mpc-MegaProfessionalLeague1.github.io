 // ============================================================
// convert-data.js
// Скрипт для конвертации данных из data.js в JSON-файлы
// Запуск: node convert-data.js
// ============================================================

const fs = require('fs');
const path = require('path');

// ============================================================
// 1. КОПИРУЕМ ВАШИ ДАННЫЕ ИЗ DATA.JS
// ============================================================

// --- ИГРОКИ ---
const playersData = [
    { name: 'bomba69', matches: 6, k: 104, d: 60, a: 12, kd: 1.73, adr: 97, diff: 133, rating: 1.53, you: true, avatar: '' },
    { name: 'Хомяк с авт', matches: 4, k: 69, d: 60, a: 17, kd: 1.15, adr: 89, diff: 81, rating: 1.18, you: false, avatar: '' },
    { name: 'uxaxaxaxa', matches: 6, k: 85, d: 69, a: 11, kd: 1.23, adr: 84, diff: 66, rating: 1.24, you: false, avatar: '' },
    { name: 'Gonduras228', matches: 6, k: 74, d: 75, a: 24, kd: 0.99, adr: 77, diff: 35, rating: 1.05, you: false, avatar: '' },
    { name: 'abdalbek', matches: 6, k: 86, d: 97, a: 21, kd: 0.89, adr: 84, diff: -30, rating: 0.89, you: false, avatar: '' },
    { name: 'wadsf', matches: 5, k: 58, d: 61, a: 14, kd: 0.95, adr: 68, diff: -60, rating: 0.90, you: false, avatar: '' },
    { name: 'AFKE', matches: 5, k: 33, d: 53, a: 21, kd: 0.62, adr: 61, diff: -15, rating: 0.83, you: false, avatar: '' },
    { name: 'Adjiare Gudju', matches: 4, k: 59, d: 69, a: 17, kd: 0.86, adr: 73, diff: -35, rating: 0.96, you: false, avatar: '' },
    { name: 'balchuqov_valera', matches: 2, k: 15, d: 31, a: 5, kd: 0.48, adr: 49, diff: -79, rating: 0.62, you: false, avatar: '' },
    { name: 'baksyhanovvand...', matches: 4, k: 25, d: 75, a: 8, kd: 0.33, adr: 41, diff: -119, rating: 0.35, you: false, avatar: '' },
    { name: 'andrejlybisin3', matches: 1, k: 7, d: 12, a: 1, kd: 0.58, adr: 38, diff: -38, rating: -0.61, you: false, avatar: '' },
    { name: 'RUS_ENZO', matches: 1, k: 8, d: 16, a: 2, kd: 0.50, adr: 66, diff: -28, rating: -0.58, you: false, avatar: '' }
];

// --- ИСТОРИЯ МАТЧЕЙ ИГРОКОВ ---
const playerMatchHistory = {
    'bomba69': [
        { match: '#0 BO3', k: 37, d: 15, diff: 22, map: 'Ancient + Anubis' },
        { match: '#1', k: 10, d: 13, diff: -3, map: 'Mirage' },
        { match: '#2', k: 24, d: 4, diff: 20, map: 'Mirage' },
        { match: '#3', k: 12, d: 7, diff: 5, map: 'Mirage' },
        { match: '#4', k: 16, d: 12, diff: 4, map: 'Mirage' },
        { match: '#5', k: 26, d: 11, diff: 15, map: 'Mirage' }
    ],
    'Хомяк с авт': [
        { match: '#0 BO3', k: 35, d: 19, diff: 16, map: 'Ancient + Anubis' },
        { match: '#1', k: 14, d: 9, diff: 5, map: 'Mirage' },
        { match: '#2', k: 14, d: 15, diff: -1, map: 'Mirage' },
        { match: '#5', k: 6, d: 17, diff: -11, map: 'Mirage' }
    ],
    'abdalbek': [
        { match: '#0 BO3', k: 20, d: 30, diff: -10, map: 'Ancient + Anubis' },
        { match: '#1', k: 8, d: 13, diff: -5, map: 'Mirage' },
        { match: '#2', k: 16, d: 11, diff: 5, map: 'Mirage' },
        { match: '#3', k: 15, d: 13, diff: 2, map: 'Mirage' },
        { match: '#4', k: 16, d: 14, diff: 2, map: 'Mirage' },
        { match: '#5', k: 11, d: 16, diff: -5, map: 'Mirage' }
    ],
    'uxaxaxaxa': [
        { match: '#0 BO3', k: 34, d: 16, diff: 18, map: 'Ancient + Anubis' },
        { match: '#1', k: 17, d: 5, diff: 12, map: 'Mirage' },
        { match: '#2', k: 10, d: 10, diff: 0, map: 'Mirage' },
        { match: '#3', k: 13, d: 9, diff: 4, map: 'Mirage' },
        { match: '#4', k: 11, d: 14, diff: -3, map: 'Mirage' },
        { match: '#5', k: 23, d: 7, diff: 16, map: 'Mirage' }
    ],
    'Gonduras228': [
        { match: '#0 BO3', k: 16, d: 32, diff: -16, map: 'Ancient + Anubis' },
        { match: '#1', k: 6, d: 13, diff: -7, map: 'Mirage' },
        { match: '#2', k: 9, d: 9, diff: 0, map: 'Mirage' },
        { match: '#3', k: 14, d: 10, diff: 4, map: 'Mirage' },
        { match: '#4', k: 13, d: 14, diff: -1, map: 'Mirage' },
        { match: '#5', k: 18, d: 13, diff: 5, map: 'Mirage' }
    ],
    'AFKE': [
        { match: '#0 BO3', k: 14, d: 24, diff: -10, map: 'Ancient + Anubis' },
        { match: '#1', k: 8, d: 5, diff: 3, map: 'Mirage' },
        { match: '#2', k: 6, d: 15, diff: -9, map: 'Mirage' },
        { match: '#5', k: 11, d: 9, diff: 2, map: 'Mirage' }
    ],
    'wadsf': [
        { match: '#1', k: 13, d: 6, diff: 7, map: 'Mirage' },
        { match: '#2', k: 10, d: 14, diff: -4, map: 'Mirage' },
        { match: '#3', k: 17, d: 9, diff: 8, map: 'Mirage' },
        { match: '#4', k: 11, d: 15, diff: -4, map: 'Mirage' },
        { match: '#5', k: 7, d: 17, diff: -10, map: 'Mirage' }
    ],
    'Adjiare Gudju': [
        { match: '#0 BO3', k: 24, d: 28, diff: -4, map: 'Ancient + Anubis' },
        { match: '#3', k: 13, d: 15, diff: -2, map: 'Mirage' },
        { match: '#4', k: 11, d: 13, diff: -2, map: 'Mirage' }
    ],
    'balchuqov_valera': [
        { match: '#3', k: 6, d: 14, diff: -8, map: 'Mirage' },
        { match: '#4', k: 9, d: 17, diff: -8, map: 'Mirage' }
    ],
    'baksyhanovvand...': [
        { match: '#0 BO3', k: 14, d: 31, diff: -17, map: 'Ancient + Anubis' },
        { match: '#1', k: 0, d: 13, diff: -13, map: 'Mirage' },
        { match: '#2', k: 3, d: 15, diff: -12, map: 'Mirage' },
        { match: '#3', k: 1, d: 14, diff: -13, map: 'Mirage' },
        { match: '#4', k: 4, d: 17, diff: -13, map: 'Mirage' }
    ],
    'andrejlybisin3': [
        { match: '#5', k: 7, d: 12, diff: -5, map: 'Mirage' }
    ],
    'RUS_ENZO': [
        { match: '#5', k: 8, d: 16, diff: -8, map: 'Mirage' }
    ]
};

// --- МАТЧИ (упрощённая версия) ---
const matchData = {
    match0: {
        id: 'match0',
        title: '🏆 Матч #0 — BO3',
        date: '30.07.2026',
        map: 'Ancient + Anubis',
        score: '2:0',
        winner: 'Team Fortress 2',
        loser: 'Team Magic',
        detail: 'BO3 · 2 карты',
        teams: {
            win: {
                name: 'Team Fortress 2',
                players: [
                    { name: 'Хомяк с авт', k: 35, d: 19, a: 5, adr: 101, highlight: true },
                    { name: 'uxaxaxaxa', k: 34, d: 16, a: 9, adr: 103 },
                    { name: 'bomba69', k: 37, d: 15, a: 3, adr: 97 },
                    { name: 'AFKE', k: 14, d: 24, a: 12, adr: 53 }
                ]
            },
            loss: {
                name: 'Team Magic',
                players: [
                    { name: 'Adjiare Gudju', k: 24, d: 28, a: 4, adr: 66 },
                    { name: 'abdalbek', k: 20, d: 30, a: 8, adr: 70 },
                    { name: 'Gonduras228', k: 16, d: 32, a: 8, adr: 58 },
                    { name: 'baksyhanovvand...', k: 14, d: 31, a: 5, adr: 49, worst: true }
                ]
            }
        }
    },
    match1: {
        id: 'match1',
        title: 'Матч #1',
        date: '09.05.2026',
        map: 'Mirage',
        score: '13:1',
        winner: 'Team 2',
        loser: 'Team 1',
        detail: 'Карта: Mirage',
        teams: {
            win: {
                name: 'Team 2 — WIN',
                players: [
                    { name: 'uxaxaxaxa', k: 17, d: 5, a: 3, adr: 131, highlight: true },
                    { name: 'Хомяк с авт', k: 14, d: 9, a: 4, adr: 105 },
                    { name: 'wadsf', k: 13, d: 6, a: 2, adr: 72 },
                    { name: 'AFKE', k: 8, d: 5, a: 4, adr: 65 }
                ]
            },
            loss: {
                name: 'Team 1 — LOSS',
                players: [
                    { name: 'abdalbek', k: 8, d: 13, a: 1, adr: 79 },
                    { name: 'bomba69', k: 10, d: 13, a: 0, adr: 95 },
                    { name: 'Gonduras228', k: 6, d: 13, a: 1, adr: 55 },
                    { name: 'baksyhanovvand...', k: 0, d: 13, a: 2, adr: 19, worst: true }
                ]
            }
        }
    },
    match2: {
        id: 'match2',
        title: 'Матч #2',
        date: '09.05.2026',
        map: 'Mirage',
        score: '3:13',
        winner: 'Team 1',
        loser: 'Team 2',
        detail: 'Карта: Mirage',
        teams: {
            win: {
                name: 'Team 1 — WIN',
                players: [
                    { name: 'bomba69', k: 24, d: 4, a: 0, adr: 117, highlight: true },
                    { name: 'abdalbek', k: 16, d: 11, a: 4, adr: 112 },
                    { name: 'Gonduras228', k: 9, d: 9, a: 5, adr: 79 },
                    { name: 'uxaxaxaxa', k: 10, d: 10, a: 1, adr: 64 }
                ]
            },
            loss: {
                name: 'Team 2 — LOSS',
                players: [
                    { name: 'Хомяк с авт', k: 14, d: 15, a: 2, adr: 94 },
                    { name: 'wadsf', k: 10, d: 14, a: 1, adr: 50 },
                    { name: 'AFKE', k: 6, d: 15, a: 2, adr: 65 },
                    { name: 'baksyhanovvand...', k: 3, d: 15, a: 2, adr: 28, worst: true }
                ]
            }
        }
    },
    match3: {
        id: 'match3',
        title: 'Матч #3',
        date: '17.05.2026',
        map: 'Mirage',
        score: '3:13',
        winner: 'Team 1',
        loser: 'Team 2',
        detail: 'Карта: Mirage',
        teams: {
            win: {
                name: 'Team 1 — WIN',
                players: [
                    { name: 'wadsf', k: 17, d: 9, a: 6, adr: 114, highlight: true },
                    { name: 'Gonduras228', k: 14, d: 10, a: 3, adr: 90 },
                    { name: 'bomba69', k: 12, d: 7, a: 3, adr: 89 },
                    { name: 'uxaxaxaxa', k: 13, d: 9, a: 1, adr: 74 }
                ]
            },
            loss: {
                name: 'Team 2 — LOSS',
                players: [
                    { name: 'abdalbek', k: 15, d: 13, a: 5, adr: 112 },
                    { name: 'fort[чилл]', k: 13, d: 15, a: 3, adr: 89 },
                    { name: 'balchuqov_valera', k: 6, d: 14, a: 2, adr: 48 },
                    { name: 'baksyhanovvand...', k: 1, d: 14, a: 1, adr: 5, worst: true }
                ]
            }
        }
    },
    match4: {
        id: 'match4',
        title: 'Матч #4',
        date: '17.05.2026',
        map: 'Mirage',
        score: '8:13',
        winner: 'Team 1',
        loser: 'Team 2',
        detail: 'Карта: Mirage',
        teams: {
            win: {
                name: 'Team 1 — WIN',
                players: [
                    { name: 'bomba69', k: 16, d: 12, a: 3, adr: 90, highlight: true },
                    { name: 'abdalbek', k: 16, d: 14, a: 1, adr: 73 },
                    { name: 'Adjiare Gudju', k: 11, d: 13, a: 5, adr: 71 },
                    { name: 'uxaxaxaxa', k: 11, d: 14, a: 2, adr: 46 }
                ]
            },
            loss: {
                name: 'Team 2 — LOSS',
                players: [
                    { name: 'Gonduras228', k: 13, d: 14, a: 7, adr: 78 },
                    { name: 'balchuqov_valera', k: 9, d: 17, a: 3, adr: 49 },
                    { name: 'wadsf', k: 11, d: 15, a: 2, adr: 49 },
                    { name: 'baksyhanovvand...', k: 4, d: 17, a: 0, adr: 18, worst: true }
                ]
            }
        }
    },
    match5: {
        id: 'match5',
        title: 'Матч #5',
        date: '01.09.2026',
        map: 'Mirage',
        score: '5:13',
        winner: 'Team 1',
        loser: 'Team 2',
        detail: 'Карта: Mirage',
        teams: {
            win: {
                name: 'Team 1 — WIN',
                players: [
                    { name: 'bomba69', k: 26, d: 11, a: 5, adr: 153, highlight: true },
                    { name: 'uxaxaxaxa', k: 23, d: 7, a: 2, adr: 127 },
                    { name: 'Gonduras228', k: 18, d: 13, a: 6, adr: 103 },
                    { name: 'AFKE', k: 11, d: 9, a: 3, adr: 67 }
                ]
            },
            loss: {
                name: 'Team 2 — LOSS',
                players: [
                    { name: 'abdalbek', k: 11, d: 16, a: 2, adr: 58 },
                    { name: 'andrejlybisin3', k: 7, d: 12, a: 1, adr: 38 },
                    { name: 'RUS_ENZO', k: 8, d: 16, a: 2, adr: 66 },
                    { name: 'wadsf', k: 7, d: 17, a: 3, adr: 55 },
                    { name: 'Хомяк с авт', k: 6, d: 17, a: 6, adr: 56, worst: true }
                ]
            }
        }
    }
};

// --- НОВОСТИ ---
const newsData = [
    { id: 1, title: '🏆 Старт сезона MPL!', text: 'Дружеский турнир MPL начался! Следите за матчами и статистикой.', date: '2026-05-09T10:00:00.000Z' },
    { id: 2, title: '🔥 Bomba69 делает 26-11!', text: 'В матче #5 Bomba69 показал невероятный результат — 26 убийств при 11 смертях!', date: new Date().toISOString() },
    { id: 3, title: '📊 Добавлен матч #5!', text: 'Новый матч против Team 2. Итог: 13:5 в нашу пользу!', date: new Date().toISOString() }
];

// ============================================================
// 2. ФУНКЦИИ ДЛЯ СОЗДАНИЯ ПАПОК И ФАЙЛОВ
// ============================================================

// Функция для создания папки (если её нет)
function ensureDir(dirPath) {
    if (!fs.existsSync(dirPath)) {
        fs.mkdirSync(dirPath, { recursive: true });
        console.log(`📁 Создана папка: ${dirPath}`);
    }
}

// Функция для сохранения JSON-файла
function saveJsonFile(filePath, data) {
    const json = JSON.stringify(data, null, 2);
    fs.writeFileSync(filePath, json, 'utf8');
    console.log(`✅ Сохранён: ${filePath}`);
}

// Функция для преобразования строки в безопасное имя файла
function slugify(text) {
    return text
        .toString()
        .toLowerCase()
        .replace(/\s+/g, '-')           // заменяем пробелы на -
        .replace(/[^\w\-]+/g, '')       // удаляем всё, кроме букв, цифр, - и _
        .replace(/\-\-+/g, '-')         // заменяем несколько - на один
        .replace(/^-+/, '')             // удаляем - в начале
        .replace(/-+$/, '');            // удаляем - в конце
}

// ============================================================
// 3. КОНВЕРТАЦИЯ ИГРОКОВ
// ============================================================

function convertPlayers() {
    const playersDir = path.join(__dirname, 'data', 'players');
    ensureDir(playersDir);

    playersData.forEach(player => {
        // Добавляем историю матчей к игроку
        const playerWithHistory = {
            ...player,
            matchHistory: playerMatchHistory[player.name] || []
        };

        const fileName = slugify(player.name) + '.json';
        const filePath = path.join(playersDir, fileName);
        saveJsonFile(filePath, playerWithHistory);
    });

    console.log(`\n✅ Конвертация игроков завершена! (${playersData.length} игроков)`);
}

// ============================================================
// 4. КОНВЕРТАЦИЯ МАТЧЕЙ
// ============================================================

function convertMatches() {
    const matchesDir = path.join(__dirname, 'data', 'matches');
    ensureDir(matchesDir);

    Object.keys(matchData).forEach(key => {
        const match = matchData[key];
        const fileName = match.id + '.json';
        const filePath = path.join(matchesDir, fileName);
        saveJsonFile(filePath, match);
    });

    console.log(`\n✅ Конвертация матчей завершена! (${Object.keys(matchData).length} матчей)`);
}

// ============================================================
// 5. КОНВЕРТАЦИЯ НОВОСТЕЙ
// ============================================================

function convertNews() {
    const newsDir = path.join(__dirname, 'data', 'news');
    ensureDir(newsDir);

    newsData.forEach(news => {
        const fileName = `news-${news.id}.json`;
        const filePath = path.join(newsDir, fileName);
        saveJsonFile(filePath, news);
    });

    console.log(`\n✅ Конвертация новостей завершена! (${newsData.length} новостей)`);
}

// ============================================================
// 6. ЗАПУСК
// ============================================================

console.log('🚀 НАЧАЛО КОНВЕРТАЦИИ ДАННЫХ\n');
console.log('📂 Создание папок и файлов...\n');

convertPlayers();
convertMatches();
convertNews();

console.log('\n🎉 ВСЁ ГОТОВО!');
console.log('📁 Данные сохранены в папке data/');
console.log('📝 Теперь вы можете настроить Decap CMS для работы с этими файлами.');
